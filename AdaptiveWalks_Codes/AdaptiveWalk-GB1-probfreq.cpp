...
//   arqteste.open("test.dat");
  ...
  minimum_path = 0;
  nwalks = 0;
  while( (minimum_path<nwalks_min) && (nwalks<1000000000) )
    {
      nwalks++;
      if( (nwalks%1000000)==0 )
      	cout << nwalks << endl;
  ...
  for( k=0; k<sequence.Nmax; k++ )
      ...
      if( cont_newpath[k]>0 )
	{
	  ...
	  if( lim_inf<0 )
	    lim_inf = 0;
	  sprintf(arq1,"FREQ-PROB-GB1-FREQUENCY-nwalks%d.dat",nwalks);
	  ptt1 = fopen(arq1,"a");
	  for( i=lim_inf; i<n; i++ )
	    {
  ...
  sprintf(arq1,"FREQ-PROB-GB1-LENGTH-nwalks%d.dat",nwalks);
  ptt1 = fopen(arq1,"a");
  ...
  sprintf(arq1,"FREQ-PROB-GB1-Predictability-nwalks%d.dat",nwalks);
  ptt1 = fopen(arq1,"a");
  for( k=0; k<sequence.Nmax; k++ )
    fprintf(ptt1," %g \t",P2[k]);
  fclose(ptt1);
  ...
  sprintf(arq1,"FREQ-PROB-GB1-Accessibility-nwalks%d.dat",nwalks);
  ptt1 = fopen(arq1,"a");
  for( k=0; k<sequence.Nmax; k++ )
  ...
  sprintf(arq1,"FREQ-PROB-GB1-Divergence-nwalks%d.dat",nwalks);
  ptt1 = fopen(arq1,"a");
  for( k=0; k<sequence.Nmax; k++ )
    fprintf(ptt1,"%g \t",path_divergence[k]);
  fclose(ptt1);

  sprintf(arq1,"FREQ-PROB-GB1-LocalOptimum-nwalks.dat");
  ptt1 = fopen(arq1,"a");
  for( k=0; k<sequence.Nmax; k++ )
    fprintf(ptt1,"%g \t",sequence.fitness[sequence.max[k]]);
  fclose(ptt1);

  sprintf(arq1,"FREQ-PROB-Relacao-GB1.dat");
  ptt1 = fopen(arq1,"a");
  for( k=0; k<sequence.Nmax; k++ )
...
void dynamics(sequencia *sequence, populacao *popul, caminhada *walk)
{
  ...
  double fmaior, r, soma_prob, soma, s[100];
	  ...
	  if( sequence->fitness[sequence->C[ind][j]]>sequence->fitness[ind] )
	    {
	      ...
	      s[nmaior] = sequence->fitness[sequence->C[ind][j]] - sequence->fitness[ind];
	      soma_prob += s[nmaior];
	      nmaior++;	
	    }
	}
